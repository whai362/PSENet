...
def evaluate_method(evaluationParams):
    """
    ...
    def transcription_match(transGt,transDet,specialCharacters=u'!?.:,*"()·[]/\'',onlyRemoveFirstLastCharacterGT=True):
        if onlyRemoveFirstLastCharacterGT:
            #special characters in GT are allowed only at initial or final position
            if transGt == transDet:
                return True
            specialCharacters = specialCharacters.decode('utf8')
        ...
        def get_ann(gt_path):
            ...
            for i, lines in enumerate(data_polygt):
                ...
                for i in range(point_num):
                    bbox.append(arr[i][0])
                    bbox.append(arr[i][1])
                ann.append((bbox, word.lower()))

            return ann

        gt_names = [gt_name for gt_name in mmcv.utils.scandir(gt_root, '.mat')]
        gt = {}
        for gt_name in gt_names:
            gt_path = gt_root + gt_name
            key = gt_name.split('.')[0].split('_')[-1]
            gt[key] = get_ann(gt_path)

        return gt

    def load_pred(pred_root):
        pred_names = [pred_name for pred_name in mmcv.utils.scandir(pred_root, '.txt')]
        ...
        for pred_name in pred_names:
            pred_path = pred_root + pred_name
            pred = get_pred(pred_path)
            key = pred_name.split('.')[0]
            subm[key] = pred
    ...
    # gt = rrc_evaluation_funcs.load_zip_file(gtFilePath,evaluationParams['GT_SAMPLE_NAME_2_ID'])
    # subm = rrc_evaluation_funcs.load_zip_file(submFilePath,evaluationParams['DET_SAMPLE_NAME_2_ID'],True)
    gt = load_tt_gt('../../data/total_text/Groundtruth/Polygon/Test/')
    subm = load_pred('../../outputs/submit_tt_rec/')
    ...
    for resFile in gt:
        ...
        evaluationLog = ""
        for n in range(len(gtFile)):
            points = gtFile[n][0]
            transcription = gtFile[n][1]

            dontCare = transcription == "###"

            gtPol = polygon_from_list(points)
            gtPols.append(gtPol)
        ...
        if resFile in subm:
            ...
            for n in range(len(detFile)):
                points = detFile[n][0]
                points = np.array(points).reshape(-1, 2)[:, ::-1].tolist()
                transcription = detFile[n][1]
                
                detPol = polygon_from_list(points)
