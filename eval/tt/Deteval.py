...
def detection_filtering(detections, groundtruths, threshold=0.5):
    for gt_id, gt in enumerate(groundtruths):
        if (gt[5] == "#") and (gt[1].shape[1] > 1):
            gt_x = np.squeeze(gt[1]).astype(int)
            gt_y = np.squeeze(gt[3]).astype(int)
            ...
            for det_id, detection in enumerate(detections):
                ...
                # detection = detection[0:-1].astype(int)
                detection = np.array(detection, dtype=np.int)
                det_y = detection[0::2]
                det_x = detection[1::2]
    ...
    for gt_id in range(num_gt):
        ...
        if num_non_zero_in_sigma >= k:
            ...
            if num_qualified_tau_candidates == 1:
                ...
                global_accumulative_precision = (
                    global_accumulative_precision + num_qualified_tau_candidates * fsc_k
                )

                local_accumulative_recall = local_accumulative_recall + fsc_k
                local_accumulative_precision = (
                    local_accumulative_precision + num_qualified_tau_candidates * fsc_k
                )
    ...
    for det_id in range(num_det):
        ...
        if num_non_zero_in_tau >= k:
            ...
            if num_qualified_sigma_candidates == 1:
                ...
                global_accumulative_recall = (
                    global_accumulative_recall + num_qualified_sigma_candidates * fsc_k
                )
                global_accumulative_precision = global_accumulative_precision + fsc_k

                local_accumulative_recall = (
                    local_accumulative_recall + num_qualified_sigma_candidates * fsc_k
                )
